...
/* Signature Menu: chỉ hiện món nổi bật */
...
/* Góc chuyện trò: chỉ hiện bài ghim, tối đa 2 bài */
content = content.replace(
...
/* Xóa toàn bộ ReservationSection đang lỗi bằng cách cắt từ function ReservationSection đến function ReservationField */
const start = content.indexOf("function ReservationSection");
const end = content.indexOf("function ReservationField");

if (start === -1) {
  throw new Error("Không tìm thấy function ReservationSection");
}

if (end === -1 || end <= start) {
  throw new Error("Không tìm thấy function ReservationField sau ReservationSection");
}

content = content.slice(0, start) + newReservationSection + "\n\n" + content.slice(end);

fs.writeFileSync(filePath, content, "utf8");

console.log("Đã sửa sạch ReservationSection và cập nhật HomePage.");
