...
// Đảm bảo có useRef để hỗ trợ vuốt ảnh.
code = code.replace(
...
const replacement = String.raw`function useDogModalViewport(enabled) {
  useEffect(() => {
    ...
    const previous = {
      modalTop: root.style.getPropertyValue("--yepo-dog-modal-top"),
      modalHeight: root.style.getPropertyValue("--yepo-dog-modal-height"),
    ...
    function applyViewport() {
      ...
      // Chrome/Safari mobile thật có thanh địa chỉ đè lên fixed modal.
      // Nếu bạn thấy khoảng cách hơi nhiều/ít, chỉnh số 92 này.
      const browserTopGap =
        isSmallMobile && isTouchDevice && !isStandaloneMode() ? 92 : 0;

      const top = Math.max(offsetTop, browserTopGap);
      const adjustedHeight = Math.max(360, height - Math.max(0, top - offsetTop));

      root.style.setProperty("--yepo-dog-modal-top", Math.round(top) + "px");
      root.style.setProperty("--yepo-dog-modal-height", Math.round(adjustedHeight) + "px");
    }
    ...
    return () => {
      ...
      root.style.setProperty("--yepo-dog-modal-top", previous.modalTop);
      root.style.setProperty("--yepo-dog-modal-height", previous.modalHeight);
...
function DogDetailModal({ dog, cardTheme, genderTheme, onClose }) {
  ...
  return (
    <div
      className="fixed left-0 z-[99999] flex w-screen max-w-[100vw] overflow-hidden bg-[#2d2015]/75 p-0 backdrop-blur-sm sm:inset-0 sm:h-auto sm:items-center sm:justify-center sm:p-5"
      style={{
        top: "var(--yepo-dog-modal-top, 0px)",
        height: "var(--yepo-dog-modal-height, 100dvh)",
      }}
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    >
      <section
        ...
        className="relative flex h-full max-h-full w-screen max-w-[100vw] flex-col overflow-hidden rounded-none border-0 shadow-[0_28px_90px_rgba(0,0,0,.28)] sm:h-auto sm:max-h-[92dvh] sm:max-w-6xl sm:rounded-[34px] sm:border-[6px] sm:border-white"
        onClick={(event) => event.stopPropagation()}
      >
        <header
          className={cn(
            "z-30 flex shrink-0 items-center justify-between gap-3 px-4 py-3 sm:px-5",
            cardTheme.ribbon
          )}
        >
                  ...
                  {activeMedia ? (
                    isVideoMedia(activeMedia) ? (
                      <video
                        ...
                        controls
                        playsInline
                        className="block max-h-[64dvh] min-h-[240px] w-full bg-[#2d2015] object-contain sm:max-h-[72vh]"
                      />
                    ) : (
                      <img
                        key={activeMedia.url}
                    ...
                    <div className="grid min-h-[260px] w-full place-items-center text-white">
                      <PawPrint size={64} />
                    </div>
                  )}
                </div>
...
console.log("✅ Đã sửa modal hồ sơ cún tránh bị thanh link mobile che:", dogCardFile);
