...
/* -------------------------------------------------------
   1. Đảm bảo MenuCategory model có sortOrder/order
------------------------------------------------------- */

const modelPath = path.resolve(process.cwd(), "server/models/MenuCategory.js");

if (fs.existsSync(modelPath)) {
  let modelContent = read(modelPath);

  if (!modelContent.includes("sortOrder")) {
  ...
  order: {
    ...
    write(modelPath, modelContent);
    console.log("Đã thêm sortOrder/order vào MenuCategory model.");
  } else {
    console.log("MenuCategory model đã có sortOrder.");
  }
} else {
  console.log("Không tìm thấy MenuCategory model, bỏ qua bước model.");
}

/* -------------------------------------------------------
   2. Thêm PATCH /api/categories/reorder vào category route
------------------------------------------------------- */

const routePath = path.resolve(process.cwd(), "server/routes/category.routes.js");

if (fs.existsSync(routePath)) {
  let routeContent = read(routePath);

  if (!routeContent.includes('"/reorder"') && !routeContent.includes("'/reorder'")) {
    const importMatch =
      routeContent.match(/import\s+(\w+)\s+from\s+["'][^"']*MenuCategory\.js["'];?/) ||
      routeContent.match(/import\s+\{\s*(\w+)\s*\}\s+from\s+["'][^"']*MenuCategory\.js["'];?/);

    const modelName = importMatch?.[1] || "MenuCategory";
...
`;
    ...
    const routerMarker =
      routeContent.match(/const\s+router\s*=\s*express\.Router\(\);\s*/) ||
      routeContent.match(/const\s+router\s*=\s*Router\(\);\s*/);

    if (!routerMarker) {
      throw new Error("Không tìm thấy dòng khai báo router trong category.routes.js");
    }

    routeContent = routeContent.replace(routerMarker[0], routerMarker[0] + reorderRoute);

    write(routePath, routeContent);
    console.log("Đã thêm route PATCH /api/categories/reorder.");
  } else {
    console.log("category.routes.js đã có route reorder.");
  }
} else {
  console.log("Không tìm thấy category.routes.js, bỏ qua bước route.");
}

/* -------------------------------------------------------
   3. Ghi đè CategoryPanel với kéo thả + nút lên/xuống
------------------------------------------------------- */

const panelPath = path.resolve(process.cwd(), "src/admin/menu/CategoryPanel.jsx");

const panelContent = `import {
  ArrowDown,
  ArrowUp,
  Edit3,
  Eye,
...
function getCategorySort(item, index) {
  return Number(item?.sortOrder || item?.order || index + 1 || 999);
}

function sortCategories(categories = []) {
  return [...categories].sort((a, b) => {
    const sortA = getCategorySort(a, 999);
    const sortB = getCategorySort(b, 999);
...
function countProducts(category, products = []) {
...
export function CategoryPanel(props) {
  ...
  const editingId = props.editingId || props.editingCategoryId || "";
  const submitting = props.submitting || props.saving || props.loading || false;
  ...
  const categorySignature = useMemo(() => {
    return categories
      .map((item, index) => \`\${getId(item)}:\${getCategorySort(item, index)}:\${getCategoryName(item)}\`)
      .join("|");
  }, [categories]);
      ...
      >
        <div className="mb-4 flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm leading-6 text-[#756144]">
              Tạo nhóm món như Coffee, Matcha, Kem cún... Thứ tự có thể kéo thả ở danh sách bên dưới.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_160px]">
          ...
          <label className="block">
            ...
            <input
              value={form.sortOrder || form.order || ""}
              onChange={(event) =>
        ...
        {orderedCategories.length === 0 ? (
          ...
          <div className="space-y-2">
            {orderedCategories.map((category, index) => {
              ...
              const productCount = countProducts(category, products);
...
write(panelPath, panelContent);
console.log("Đã ghi đè CategoryPanel với chức năng kéo thả danh mục.");
