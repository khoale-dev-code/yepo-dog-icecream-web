...
function MediaCarousel({ media = [], mode = "modal" }) {
  ...
  if (isPageMode) {
    return (
      <div
        className="relative isolate w-full overflow-hidden bg-[#111] select-none"
      ...
      >
        {currentMedia.type === "video" ? (
          <video
            ...
            controls
            playsInline
            className="block max-h-[82dvh] min-h-[280px] w-full bg-[#111] object-contain"
          />
        ) : (
          <img
            key={currentMedia.url}
            ...
            draggable={false}
            className="block h-auto w-full bg-[#111] object-contain"
          />
        )}
...
console.log("✅ Fixed mobile post image full height:", postsFile);
