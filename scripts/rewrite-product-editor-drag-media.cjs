...
function getExistingMediaKey(media, index) {
  return \`existing:\${media?.publicId || media?.public_id || media?.url || media?.secureUrl || index}\`;
}
...
export function ProductEditorPanel({
  ...
  const existingPreviews = useMemo(() => {
    return existingMedia.map((media, index) => ({
      kind: "existing",
  ...
  useEffect(() => {
    const keys = mediaItems.map((item) => item.key);

    setForm((current) => {
      const currentOrder = Array.isArray(current.mediaOrder) ? current.mediaOrder : [];
      const nextOrder = [
        ...currentOrder.filter((key) => keys.includes(key)),
        ...keys.filter((key) => !currentOrder.includes(key)),
      ];
  ...
  }, [setForm, mediaItems.map((item) => item.key).join("|")]);
  ...
  function removeMediaItem(item) {
    const nextItems = orderedMediaItems.filter((mediaItem) => mediaItem.key !== item.key);
    commitMediaOrder(nextItems);
  }
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-2 text-sm font-normal leading-6 text-[#756144]">
                Bấm dấu cộng để mở form. Thứ tự món mới đang gợi ý là #{productCount + 1}.
              </p>
            </div>
          </div>
        ...
        <FormBlock icon={ImagePlus} title="Hình ảnh minh họa">
          <label className="block cursor-pointer rounded-[22px] border border-dashed border-[#d8b77e] bg-[#FFFAFA] p-5 text-center transition hover:bg-[#f7efe3]">
            ...
            <p className="mt-1 text-xs font-normal text-[#756144]">
              {files.length
                ? \`\${files.length} file mới đã chọn. Kéo thả ảnh để đổi thứ tự.\`
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
                {orderedMediaItems.map((item, index) => (
                  <MediaPreview
                    ...
                    index={index}
                    dragIndex={dragIndex}
                    label={item.kind === "existing" ? "Hiện tại" : \`Mới \${index + 1}\`}
          ...
          {files.length > 0 && (
            <button
              type="button"
              onClick={() => {
                const nextItems = orderedMediaItems.filter((item) => item.kind !== "file");
                commitMediaOrder(nextItems);
              }}
            ...
            {(form.sizes || []).map((size, index) => (
              ...
              >
                ...
                <div className="grid grid-cols-[1fr_1fr_44px] gap-2">
                  ...
                  <input
                    value={size.oldPrice}
                    onChange={(event) => updateSize(index, "oldPrice", event.target.value)}
                    placeholder="Giá cũ"
                    inputMode="numeric"
                    className="h-11 min-w-0 rounded-xl border border-[#d8b77e] bg-white px-3 text-sm outline-none focus:border-[#b98c49]"
                  />
...
function Field({ label, value, onChange, required, inputMode, placeholder, helper }) {
  return (
    <label className="block">
      <span className="mb-2 block text-sm font-brand text-[#3b2a18]">
        {label}
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 px-2 py-2">
        <div className="min-w-0">
          ...
          <p className="truncate text-[10px] text-[#756144]">
            {item.name}
          </p>
        </div>
...
console.log("Đã cập nhật ProductEditorPanel: kéo thả ảnh, xóa ảnh, chọn nhiều ảnh, ảnh đầu là ảnh đại diện.");
