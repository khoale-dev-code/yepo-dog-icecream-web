...
function MediaCarousel({ media = [], mode = "modal" }) {
  ...
  if (!currentMedia) {
    return (
      <div className="grid h-full min-h-[280px] place-items-center bg-[#111] text-sm text-white sm:min-h-[320px]">
        Không có media
      </div>
    );
  }
      ...
      {canMove && (
        <>
          <button
            type="button"
            onClick={goPrev}
            className="absolute left-3 top-1/2 grid h-9 w-9 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-[#2D2D2D] shadow-lg transition hover:bg-white sm:h-10 sm:w-10"
          ...
          <button
            type="button"
            onClick={goNext}
            className="absolute right-3 top-1/2 grid h-9 w-9 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-[#2D2D2D] shadow-lg transition hover:bg-white sm:h-10 sm:w-10"
...
function MobilePostDetail({ post, shop, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex flex-col bg-[#FFFAFA]">
      <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-[#b98c49]/15 bg-white/95 px-3 py-3 backdrop-blur">
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        <section className="-mt-4 relative z-10 rounded-t-[2rem] bg-[#FFFAFA] px-4 pb-8 pt-5 shadow-[0_-18px_40px_rgba(185,140,73,0.08)]">
