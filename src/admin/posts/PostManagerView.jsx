...
export function PostManagerView() {
  ...
  const stats = useMemo(() => {
    return {
      total: posts.length,
      published: posts.filter((post) => post.isPublished !== false && post.isActive !== false).length,
      hidden: posts.filter((post) => post.isPublished === false || post.isActive === false).length,
      pinned: posts.filter((post) => post.isPinned === true).length,
      media: posts.filter((post) => normalizePostMedia(post.media).length > 0).length,
    };
  }, [posts]);
  ...
  const filteredPosts = useMemo(() => {
    ...
    return posts
      .filter((post) => {
        ...
        const published = post.isPublished !== false && post.isActive !== false;
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const payload = buildPostPayload({
        form,
        media,
        orderFallback: editingPost ? editingPost.order || 999 : posts.length + 1,
      });
      ...
      if (editingPost) {
        ...
        setNotice({ type: "success", text: payload.isPublished ? "Đã đăng bài." : "Đã lưu bài ở trạng thái ẩn." });
      }
  ...
  async function handleTogglePublished(post) {
    try {
      const currentPublished = post.isPublished !== false && post.isActive !== false;
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-[#756144]">
              Đăng món mới, ưu đãi, hình ảnh không gian hoặc thông báo nhanh cho khách hàng.
            </p>
          </div>
...
function PreviewPanel({ form, mediaDrafts }) {
  const firstMedia = mediaDrafts[0];
  const hasDraft = form.content.trim() || mediaDrafts.length > 0;

  return (
    <section className="rounded-[28px] border border-[#d8b77e]/80 bg-white p-4 shadow-[0_14px_40px_rgba(87,61,28,.05)]">
        ...
        {firstMedia && (
          <div className="aspect-video bg-white">
            {firstMedia.type === "video" ? (
              <video src={firstMedia.url} muted playsInline className="h-full w-full object-cover" />
            ) : (
        ...
        <div className="p-4">
          {hasDraft ? (
            <p className="line-clamp-6 whitespace-pre-wrap text-sm font-semibold leading-6 text-[#3b2a18]">
              {form.content.trim() || `${mediaDrafts.length} media đang chờ đăng`}
            </p>
          ) : (
...
function GuidePanel() {
  return (
    <section className="rounded-[28px] border border-[#d8b77e]/80 bg-[#FFFAFA] p-4">
